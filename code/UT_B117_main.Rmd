...
# Early estimate: Jan 16- Feb. 12
c1 = -2.861
c1_distrib<-runif(n= nsamps, min = -2.9, max = -2.83)
k1 = 0.077
k1_distrib<-runif(n= nsamps, min = 0.017, max = 0.140)

# Full estimate: Jan 16- April 9
c2 = c1
c2_distrib<-runif(n= nsamps, min = -2.9, max = -2.83)
k2 = 0.040
k2_distrib = runif(n=nsamps, min = 0.037, max = 0.043)
p_curr = 0.627
p_curr_distrib<-runif(nsamps, 0.501, 0.739)

write.csv(c1_distrib, '../data/c1_distrib.csv', row.names = FALSE)
write.csv(k1_distrib,'../data/k1_distrib.csv', row.names = FALSE)
write.csv(c2_distrib, '../data/c2_distrib.csv', row.names = FALSE)
write.csv(k2_distrib,'../data/k2_distrib.csv', row.names = FALSE)
write.csv(p_curr_distrib, '../data/p_curr_distrib.csv', row.names = FALSE)

M1_distrib = exp(mean_GI*k1_distrib)
M2_distrib = exp(mean_GI*k2_distrib) # This should probably be a function of b0 and b1? 
...
```{r}
k1_distrib<-read.csv('../data/k1_distrib.csv')
k1_distrib<-k1_distrib$x
c1_distrib<-read.csv('../data/c1_distrib.csv')
c1_distrib<-c1_distrib$x
k2_distrib<-read.csv('../data/k2_distrib.csv')
k2_distrib<-k2_distrib$x
c2_distrib<-read.csv('../data/c2_distrib.csv')
c2_distrib<-c2_distrib$x
...
out_list<-run_spring_SEIR(spring_par_table, case_data_spring, Rt_summary_spring, iRts, spring_last_date, cumIJan, k2_distrib, c2_distrib, p_curr_distrib)
...
kl_distrib<-rtri(n = nsamps, min = log(1.56)/5.5, max = log(1.63)/5.5, mode = log(1.59)/5.5)
out_list<-run_spring_SEIR(spring_par_table, case_data_spring, Rt_summary_spring, iRts, spring_last_date, cumIJan, kl_distrib, c2_distrib, p_curr_distrib)
...
plot1<-ggplot() + geom_line(data = df_t_sim, aes(x = dates, y = 1000*I_variant/N, group = samples), alpha = 0.05, color = "tan1") + geom_line(data = df_t_sim, aes(x = dates, y = 1000*I_variant_lit/N, group = samples), alpha = 0.05, color = "cyan4")+
  geom_line(data = df_t_sim, aes(x = dates, y = 1000*I_novariant/N, group = samples), alpha = 0.02, color = "mediumpurple") +
  geom_line(data = df_t, aes(x = springdates, y = 1000*Imediannv/N), size = 1, color = "mediumpurple")+ geom_line(data = df_t, aes(x = springdates, y = 1000*Imedianv/N), size = 1, color = "tan1") +geom_line(data = df_t, aes(x = springdates, y = 1000*Imedian_lit/N), size = 1, color = "cyan4")+
...
plot4<-ggplot() + geom_line(data = df_t, aes(x = springdates, y = 1000*Imediannv/N, group = scenario, linetype = scenario),color = "mediumpurple", size = 1) + 
  geom_line(data = df_t, aes(x = springdates, y = 1000*Imedianv/N, group = scenario, linetype = scenario), color = "tan1", size = 1) +geom_line(data = df_t, aes(x = springdates, y = 1000*Imedian_lit/N, group = scenario, linetype = scenario), color = "cyan4", size = 1)+
